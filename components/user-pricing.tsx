    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
                ...
                <p className="text-muted-foreground animate-pulse text-lg font-medium">Curating your plans...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
                ...
                <h3 className="text-2xl font-bold tracking-tight">Oops! Something went wrong</h3>
        ...
        <div className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            <div className="text-center mb-16 space-y-4">
                ...
                <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
                    Scale your business with our flexible pricing models. No hidden fees, just pure value.
                </p>

                <div className="flex items-center justify-center mt-8 space-x-4">
                    <span className={`text-sm font-medium ${billingCycle === 'month' ? 'text-foreground' : 'text-muted-foreground'}`}>Monthly</span>
                    <button
                    ...
                    >
                        <div className={`w-5 h-5 bg-primary rounded-full transition-transform duration-200 ease-in-out ${billingCycle === 'year' ? 'translate-x-[28px]' : 'translate-x-0'}`} />
                    </button>
                    <span className={`text-sm font-medium ${billingCycle === 'year' ? 'text-foreground' : 'text-muted-foreground'}`}>
                        Yearly <span className="text-xs font-bold text-green-500 bg-green-500/10 px-2 py-0.5 rounded-full ml-1">Save 20%</span>
                    </span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {pricing.map((product) => {
                    const price = product.prices.find((p) => p.interval === billingCycle) || product.prices[0];
                    const isPro = product.name.toLowerCase() === 'pro';

                    return (
                        <div
                            key={product.id}
                            className={`relative flex flex-col p-8 rounded-3xl border transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 ${isPro
                        ...
                        >
                            ...
                            <div className="mb-8">
                                ...
                                <p className="text-muted-foreground mt-2 min-h-[3rem] line-clamp-2">
                                    {product.description || `Unlock core features with the ${product.name} plan.`}
                                </p>
                            </div>
                            ...
                            <div className="mb-8 flex items-baseline gap-1">
                                ...
                                <span className="text-muted-foreground font-medium">/{billingCycle}</span>
                            </div>

                            <div className="space-y-4 mb-10 flex-grow">
                                {["Unlimited Projects", "Premium Support", "Advanced Analytics", "Custom Domains"].map((feature) => (
                            ...
                            <button
                                className={`w-full py-4 rounded-2xl font-bold transition-all transform active:scale-[0.98] ${isPro
